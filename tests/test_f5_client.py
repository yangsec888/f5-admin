...
class TestF5Client(unittest.TestCase):
    ...
    def test_search_cert_in_client_ssl_profile(self):
        val = ['    app-service none', '    cert wildcard.dev.penguinrandomhouse.com-clientssl.crt', '    cert-key-chain {', '        wildcard_wildcard {', '            cert wildcard.dev.penguinrandomhouse.com-clientssl.crt', '            chain wildcard.dev.penguinrandomhouse.com-clientssl.crt', '            key wildcard.dev.penguinrandomhouse.com-clientssl.key', '        }', '    }', '    chain wildcard.dev.penguinrandomhouse.com-clientssl.crt', '    defaults-from clientssl', '    destination-ip-blacklist none', '    destination-ip-whitelist none', '    hostname-blacklist none', '    hostname-whitelist none', '    inherit-certkeychain false', '    key wildcard.dev.penguinrandomhouse.com-clientssl.key', '    passphrase none', '    source-ip-blacklist none', '    source-ip-whitelist none']
        self.assertEqual(self.client.search_cert_in_client_ssl_profile(val),'wildcard.dev.penguinrandomhouse.com-clientssl.crt')

    def test_search_client_ssl(self):
        val = ['    description "sam li test"', '    destination 10.106.220.50:https', '    ip-protocol tcp', '    mask 255.255.255.255', '    profiles {', '        http-x-forwarded-for { }', '        serverssl-insecure-compatible {', '            context serverside', '        }', '        tcp { }', '        wildcard.dev.penguinrandomhouse.com-clientssl {', '            context clientside', '        }', '    }', '    rules {', '        niq-security-block', '    }', '    security-log-profiles {', '        splunk-logging', '        splunk-logging_local', '    }', '    source 0.0.0.0/0', '    translate-address enabled', '    translate-port enabled', '    vlans {', '        Vlan422-WebSrvr-10.106.222.x', '        dev_int_vlan703', '    }', '    vlans-enabled', '    vs-index 4']
        self.assertEqual(self.client.search_client_ssl(val),'wildcard.dev.penguinrandomhouse.com-clientssl')
